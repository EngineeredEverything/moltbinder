...
app.get('/stats', (req, res) => {
    const coalitions = getCoalitions(db);
    
    const stats = {
        total_agents: Object.keys(db.agents).length,
        active_alliances: Object.values(db.alliances).filter(a => a.status === 'active').length,
        total_deals: Object.keys(db.deals).length,
        executed_deals: Object.values(db.deals).filter(d => d.status === 'executed').length,
        pending_requests: Object.values(db.alliance_requests).filter(r => r.status === 'pending').length,
